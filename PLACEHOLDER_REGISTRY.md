...
#   review_excerpt — the verbatim review excerpt the Drafter SELECTED for this
#     post (a substring of the full review_text). Empty for non-Social-Proof
#     posts. Persisted so the Critic can read the exact excerpt the Drafter
#     chose: the creative_hook/caption_hook overlap check (Drafter
#     validate_creative_hook_text, Critic B11) permits an overlap whose
#     overlapping portion is contained in this excerpt — faithful quotation of
#     the customer's words is not penalized. The two validators must stay in
#     lockstep on this exception.
